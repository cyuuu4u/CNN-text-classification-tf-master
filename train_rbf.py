...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        def train_svm(x_train, y_train):
            feed_dict = {cnn.input_x: x_train,
                         cnn.dropout_keep_prob: FLAGS.dropout_keep_prob}
            train_features = sess.run(cnn.h_pool_flat, feed_dict)
            # num_features = FLAGS.num_filters * 3
            ...
            #     batch_features = sess.run(cnn.h_pool_flat, {cnn.input_x: x_train_batch, cnn.dropout_keep_prob: 1.0})
        ...
        def dev_svm(x_dev,y_dev):
            feed_dict = {cnn.input_x: x_dev,
                         cnn.dropout_keep_prob: 1.0}
            dev_features = sess.run(cnn.h_pool_flat, feed_dict)
            dev_labels = y_dev.reshape(-1, )
            accuracy = clf.score(dev_features, dev_labels)
            return accuracy

        def test_svm(x_test):
            feed_dict = {cnn.input_x: x_test,
                         cnn.dropout_keep_prob: 1.0}
            test_features = sess.run(cnn.h_pool_flat, feed_dict)
        ...
        for batch in batches:
            ...
            if current_step % batches_per_epoch == 0:  # 每训练一定数量的batch后就用验证集验证一下看验证集上的loss是否有在持续下降，EARLY STOPPING以解决overfitting的问题
                # 一个epoch结束
                print("\nValidation:")
                # dev_step(x_dev, y_dev, writer=dev_summary_writer) # 没用SVM验证
                # print("")
                ########## Train svm ##########
